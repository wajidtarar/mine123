...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    } else if([segue.identifier isEqualToString:@"pagedView" ])
    {
        ...
        RootViewController *objSelectedPhoto = segue.destinationViewController;
//        PagedScrollViewController.delegate = self;
//        objSelectedPhoto.pageImages1 = objPhotosArray;
//        PagedScrollViewController = photoKey;
//        objSelectedPhoto.pageToShow = [NSNumber numberWithInt:selectedRow];
        
        int index = selectedRow;
        NSLog(@"index %i", index);
        
        [appDelegate.imagesArrayDetails removeAllObjects];
        for(int i = index; i < [[appDelegate imagesArray] count]; i = i + 1)
        {
            NSLog(@"i1 %i", i);

            if([[appDelegate imagesArray] objectAtIndex:i] !=  [NSNull null]){
                [appDelegate.imagesArrayDetails addObject:[[appDelegate imagesArray] objectAtIndex:i]];
            }
...
//                                  [appDelegate.imagesLinks objectAtIndex:indexPath.row]];
    ...
    NSMutableString *urlString1 =[NSMutableString stringWithFormat:@"%@%@&type=image", baseUrl,
                                  [appDelegate.imagesLinks objectAtIndex:indexPath.row]];
